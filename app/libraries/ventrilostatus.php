...
static function StrKey( $src, $key, &$res )
{
	$key .= " ";
...
static function StrSplit( $src, $sep, &$d1, &$d2 )
{
	$pos = strpos( $src, $sep );
	if ( $pos === false )
	{
    ...
    static function StrDecode( &$src )
{
    ...
    static function VentriloDisplayEX1( &$stat, $name, $cid, $bgidx )
{
    ...
    */
    ...
    static function VentriloDisplayEX2_Stripe( $perc, $val, $bgidx )
{
    $fgcol = "#000000";
    ...
    static function VentriloDisplayEX2( &$stat, $name, $cid, $bgidx )
{
    ...
    static function VentriloInfoEX1_Stripe( &$bgidx, $name, $val )
{
    if ( $bgidx % 2 )
        $bgcolname = "#666666";
    else
    ...
    static function VentriloInfoEX1( &$stat )
{
    $bgidx	= 0;
...
class CVentriloClient
{
	...
	function Parse( $str )
	{
		...
		{
			ventrilofunctions::StrSplit( $ary[ $i ], "=", $field, $val );
			...
			{
				$this->m_name = ventrilofunctions::StrDecode( $val );
				continue;
			}
			
			if ( strcasecmp( $field, "COMM" ) == 0 )
			{
				$this->m_comm = ventrilofunctions::StrDecode( $val );
				continue;
			}
		}
	}
...
class CVentriloStatus
{
	...
	function Parse( $str )
	{
		...
		if ( ventrilofunctions::StrKey( $str, "ERROR:", $val ) )
		{
			$this->m_error = $val;
			return -1;
		}

		if ( ventrilofunctions::StrKey( $str, "NAME:", $val ) )
		{
			$this->m_name = ventrilofunctions::StrDecode( $val );
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "PHONETIC:", $val ) )
		{
			$this->m_phonetic = ventrilofunctions::StrDecode( $val );
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "COMMENT:", $val ) )
		{
			$this->m_comment = ventrilofunctions::StrDecode( $val );
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "AUTH:", $this->m_auth ) )
			return 0;
		
		if ( ventrilofunctions::StrKey( $str, "MAXCLIENTS:", $this->m_maxclients ) )
			return 0;
		
		if ( ventrilofunctions::StrKey( $str, "VOICECODEC:", $val ) )
		{
            ventrilofunctions::StrSplit( $val, ",", $this->m_voicecodec_code, $desc );
			$this->m_voicecodec_desc = ventrilofunctions::StrDecode( $desc );
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "VOICEFORMAT:", $val ) )
		{
            ventrilofunctions::StrSplit( $val, ",", $this->m_voiceformat_code, $desc );
			$this->m_voiceformat_desc = ventrilofunctions::StrDecode( $desc );
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "UPTIME:", $val ) )
		{
			$this->m_uptime = $val;
			return 0;
		}
		
		if ( ventrilofunctions::StrKey( $str, "PLATFORM:", $val ) )
		{
			$this->m_platform = ventrilofunctions::StrDecode( $val );
			return 0;
		}
			
		if ( ventrilofunctions::StrKey( $str, "VERSION:", $val ) )
		{
			$this->m_version = ventrilofunctions::StrDecode( $val );
			return 0;
		}
			
		if ( ventrilofunctions::StrKey( $str, "CHANNELCOUNT:", $this->m_channelcount ) )
			return 0;
			
		if ( ventrilofunctions::StrKey( $str, "CHANNELFIELDS:", $this->m_channelfields ) )
			return 0;
			
		if ( ventrilofunctions::StrKey( $str, "CHANNEL:", $val ) )
		{
		...
		if ( ventrilofunctions::StrKey( $str, "CLIENTCOUNT:", $this->m_clientcount ) )
			return 0;
			
		if ( ventrilofunctions::StrKey( $str, "CLIENTFIELDS:", $this->m_clientfields ) )
			return 0;
			
		if ( ventrilofunctions::StrKey( $str, "CLIENT:", $val ) )
		{
